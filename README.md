...
1) Text input (e.g."I like coffee") is fed into word, positional, and sentence embeddings and are added together.
...
Once trained on a large corpus, BERT is _fine-tuned_ on specific tasks by addint a small classificaiton head on top of the final encoder output. During this phase, the network is updated using labeled data for tasks. More on this pipeline later.
...
OpenAI put everything they had behind these findings and cranked up scale wildly. GPT-3, released in 2020, was over one-hundred times larger than GPT-2 at 175 billion parameters and over 300 billion tokens of data (more on tokens later). 

[[INSERT MODEL SIZES]]


## Pre-Training + Post-Training: How to Build a Large Language Model
The original GPT was trained in two stages: first, a large-scaled unsupervised language modeling phase, referred to as _pre-training_, and a second supervised fine-tuning phase on specific downstream tasks like question answering. 
